...
if __name__ == '__main__':
    ...
    X_pimunu_odd = []
    X_pimunu_eve = []
    _pidar_mudar_atar_odd = _pidar_mudar_atar[_pidar_mudar_atar['atar.planeid'] % 2 != 0]
    _pidar_mudar_atar_eve = _pidar_mudar_atar[_pidar_mudar_atar['atar.planeid'] % 2 == 0]
    for field in _fields:
        _odd =  ak.fill_none(ak.pad_none(_pidar_mudar_atar_odd[field], 300, clip=True), -999)
        _odd = ak.to_numpy(_odd).reshape(len(_odd), 300, 1)        
        X_pimunu_odd += [_odd]
        _eve =  ak.fill_none(ak.pad_none(_pidar_mudar_atar_eve[field], 300, clip=True), -999)
    ...
    _target = np.concatenate(
        [
            np.ones(len(_pienu_atar)),
            np.zeros(len(_pidar_mudar_atar)),
        ]
    )
    ...
    _nn_model = rnn_model(n_vars=len(_fields))
    ...
    if do_plots:
        ...
        fig = plt.figure()
        ax = fig.add_subplot()
        ax.set_xlabel('pi- enu efficiency')
        ax.set_ylabel('pi - mu  - e efficiency')
        plt.plot(tpr, fpr)
        plt.yscale('log')
